...
namespace Two\Composer\Installer;
...
class PackageInstaller extends LibraryInstaller
{
    ...
    public function checkUsage(Composer $composer)
    {
        ...
        $postAutoloadDump = 'Two\Composer\Installer\PackageInstaller::postAutoloadDump';
     ...
     *
     * Recrée la carte du chemin du package de Two, basée sur les informations du compositeur et 
     ...
     *
     * Ajouter tous les packages composer de type Two-package, et tous les plugins situés
    ...
    public static function determinePlugins($packages, $packagesDir = 'packages', $vendorDir = 'vendor')
    {
        ...
        foreach ($packages as $package) {
            if ($package->getType() !== 'Two-package') {
                continue;
            }
        ...
        if (is_dir($packagesDir)) {
            ...
            foreach ($iterator as $info) {
                ...
                if (is_array($config) && ($config['type'] === 'Two-package')) {
...
PHP;
    ...
    {
        return $vendorDir . DIRECTORY_SEPARATOR . 'Two-packages.php';
    }
        ...
        if (is_null($namespace)) {
            throw new RuntimeException(
                sprintf(
                    ...
                    " as stated in README on https://github.com/nico2dev/Two-installer",
     ...
     *
     * Ce programme d'installation ne prend en charge que les packages de type 'Two-package'.
     *
     * @return bool
     */
    public function supports($packageType)
    {
        return ('Two-package' === $packageType);
    }

    /**
     * Installe un package spécifique.
     *
     * Une fois le package installé, le fichier de configuration `Two-packages.php` de l'application
     ...
     *
     * Une fois le package installé, le fichier de configuration `two-packages.php` de l'application est
    ...
    public function updateConfig($name, $path)
    {
        ...
        if (! isset($config)) {
            $this->io->write(
                'ERROR - `vendor/Two-packages.php` file is invalid. Packages path configuration not updated.'
            );
    ...
    /**
     * Assurez-vous que le fichier vendor/Two-packages.php existe.
     *
    ...
    protected function ensureConfigFile($path)
    {
        if (file_exists($path)) {
            if ($this->io->isVerbose()) {
                $this->io->write('vendor/Two-packages.php exists.');
            }
...
PHP;
        ...
        if ($this->io->isVerbose()) {
            $this->io->write('Created vendor/Two-packages.php');
        }
    }
